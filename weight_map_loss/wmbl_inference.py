...
def verify():
    class_weights= [1,1] #for verification
    thickness = 1

    X, y = get_data(path_train, load_masks=True)
    w_train = generate_weight_maps(y,class_weights,thickness)
    ...
    #BCE loss
    loss_bce = K.mean(binary_crossentropy(K.variable(y),K.variable(y*0.6))).eval(session=K.get_session()) #
    
    y_true =   np.stack((y,w_train),axis=0)
    y_pred =   np.stack((y*0.6,w_train),axis=0) 

    print(y_pred.shape)

    #WMB_CW loss
    loss_map_cw=cw_map_loss_verification(K.variable(y_true),K.variable(y_pred)).eval(session=K.get_session())

    print("BCE",loss_bce)   
    print("weightmap and class weighting",loss_map_cw)
    #BCE 0.039808743
    #weightmap and class weighting 0.03980883

    np.testing.assert_almost_equal(loss_bce,loss_map_cw)
#------------------------------------------
...
if(args["state"]=="train"):
    ...
    #Developer only - verify loss reduces to bce with weights = 1
    verify()
    exit()
